...
;(function($,document,window,undefined) {
    ...
    $.fn.jCounter = function(options,callback) {
        ...
        function jCslider(el,unitclassName,timeUnit,eventDate,duration) {
            $(el).find(unitclassName + " u").each(function(i,el) {
            ...
            $(el).find(unitclassName).animate({
                top: '0.15em'
            },200, function() {
                $(el).find(unitclassName + " u:eq(1)").remove();
                $(el).find(unitclassName).prepend('<u></u>');
                $(el).find(unitclassName).css({'top':'-1.24em'})
            });
        }
